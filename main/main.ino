...
bool packetSent, packetQueued;
  ...
  static uint8_t txBuffer[PAYLOAD_LENGTH];
#elif defined(PAYLOAD_USE_CAYENNE)
  // CAYENNE DF
  static uint8_t txBuffer[PAYLOAD_LENGTH] = {0x03, 0x88, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
#endif
...
#if defined(PAYLOAD_USE_FULL)
  // includes number of satellites and accuracy
  uint8_t txBuffer[PAYLOAD_LENGTH];
#elif defined(PAYLOAD_USE_CAYENNE)
  // CAYENNE DF
  static uint8_t txBuffer[PAYLOAD_LENGTH] = {0x03, 0x88, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
#endif
};
SecuredLinkedList<failurePt> failurePtList = SecuredLinkedList<failurePt>();
// -----------------------------------------------------------------------------
// Application
// -----------------------------------------------------------------------------

// isfailure means if the this represents a lora failure packet
...
void onLoraFailure() {
  ...
  failurePt newfailurept;
  buildPacket(newfailurept.txBuffer, true);
  failurePtList.push(newfailurept);
}
...
void transmitLoraFailures() {
  ...
  for(int i = 0; i < size; i++) {
    failurePt pt = failurePtList.pop();
    lmic_tx_error_t err = trySend(pt.txBuffer, sizeof(pt.txBuffer));

    if (err != 0) {
      snprintf(buffer, sizeof(buffer), "ERROR TXMIT %d\n", err);
      screen_print(buffer);
      failurePtList.push(pt);
    }
  }
}
 ...
 */
lmic_tx_error_t trySend(uint8_t* buf, int bufsize) {
    ...
    packetQueued = true;
    lmic_tx_error_t err = ttn_send(buf, bufsize, LORAWAN_PORT, confirmed);
...
void callback(uint8_t message) {
  ...
  if (EV_JOINING == message) {
    ...
    //char buffer[6];
    //snprintf(buffer, sizeof(buffer), "m=%02X", message);
    //screen_print(buffer);
  }
...
void scanI2Cdevice(void)
{
    ...
    for (addr = 1; addr < 127; addr++) {
        ...
        if (err == 0) {
          ...
          //Serial.print("Error on I2C device found at address 0x");
           // if (addr < 16)
           //     Serial.print("0");
          //  Serial.print(addr, HEX);
         //   Serial.println(" !");
...
void loop() {
  ...
  if (!digitalRead(BUTTON_PIN)) {
    ...
    if (millis() > rejoinMinPressMs) {
      ...
      if (hasValidGPSPosition()) {
        ...
        if (err != 0) {
          snprintf(buffer, sizeof(buffer), "PRTX ERR ERR %d\n", err);
          screen_print(buffer);
        }
      } else {
  ...
  if (0 == last || millis() - last > SEND_INTERVAL) {
    ...
    if (lora_joined && auto_transmit && validGPS) {
      ...
      if (err != 0) {
        snprintf(buffer, sizeof(buffer), "INTTX ERR %d\n", err);
        screen_print(buffer);
        delay(1000);
      }
      else {
