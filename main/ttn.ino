...
void ttn_join() {
    ...
    LMIC_setClockError(MAX_CLOCK_ERROR * CLOCK_ERROR / 100);
        ...
        if (cleared == true) {
            Serial.println("cleared session");
        } else {
            Serial.println("Could not clear session");
        }
...
lmic_tx_error_t ttn_send(uint8_t * data, uint8_t data_size, uint8_t port, bool confirmed){
    ...
    lmic_tx_error_t err = LMIC_setTxData2(port, data, data_size, confirmed ? 1 : 0);
